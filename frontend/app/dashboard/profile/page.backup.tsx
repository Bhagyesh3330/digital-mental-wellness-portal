    ...
    try {
      ...
      if (response?.success && response.data) {
        ...
        if (user) {
          const fallbackProfile = {
            id: user.id,
            email: user.email,
            firstName: user.firstName, // From registration form
            lastName: user.lastName,   // From registration form
            phone: user.phone,         // From registration form
            hostelName: user.hostelName,   // From registration form (if student)
            roomNumber: user.roomNumber,   // From registration form (if student)
            isActive: user.isActive,
            createdAt: user.createdAt,
            ...(user.role === 'student' && {
              studentProfile: {
                studentId: user.id.toString().padStart(6, '0'), // Generate from user ID
                enrollmentNumber: '',
                course: 'Not specified', // To be filled by user
                department: '',
                yearOfStudy: 1,
                admissionYear: new Date().getFullYear(),
                dateOfBirth: '',
          ...
          setValue('firstName', user.firstName);  // These come from registration
          setValue('lastName', user.lastName);    // These come from registration
      ...
      if (user) {
        const fallbackProfile = {
          id: user.id,
          email: user.email,
          firstName: user.firstName, // From registration
          lastName: user.lastName,   // From registration
          phone: user.phone,         // From registration
          hostelName: user.hostelName,   // From registration (if student)
          roomNumber: user.roomNumber,   // From registration (if student)
          isActive: user.isActive,
          createdAt: user.createdAt,
          ...(user.role === 'student' && {
            studentProfile: {
        ...
        setValue('firstName', user.firstName); // From registration
        setValue('lastName', user.lastName);   // From registration
        setValue('phone', user.phone || '');
        if (user.role === 'student') {
          setValue('hostelName', user.hostelName || '');
          setValue('roomNumber', user.roomNumber || '');
        ...
        >
          ...
          <form onSubmit={handleSubmit(onSubmitProfile)} className="space-y-6">
            ...
            <div>
              ...
              <div className="p-3 bg-netflix-gray-dark/30 rounded-lg text-netflix-gray-light">
            ...
            {/* Counselor-specific contact fields */}
            {user?.role === 'counselor' && (
              <div className="bg-wellness-secondary/5 border border-wellness-secondary/20 rounded-lg p-6 mb-6">
                <h4 className="text-lg font-semibold text-white mb-4 flex items-center space-x-2">
                  <Shield className="w-5 h-5 text-wellness-secondary" />
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="p-4 bg-netflix-gray-dark/30 rounded-lg text-white font-medium border-l-4 border-wellness-secondary">
                      {profile.email}
                    </div>
                  </div>
                  <div>
              ...
              <div className="border-t border-netflix-gray-dark pt-6 mt-6">
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  <div>
                    ...
                    <div className="p-3 bg-netflix-gray-dark/30 rounded-lg text-white">
                      {profile.counselorProfile.licenseNumber || 'Not provided'}
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      <input
                        {...register('profileData.yearsOfExperience', { valueAsNumber: true })}
                        type="number"
                        className="input-netflix w-full"
                      ...
                      <div className="p-3 bg-netflix-gray-dark/30 rounded-lg text-white">
                        {profile.counselorProfile.yearsOfExperience || 'Not provided'} years
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="mt-6">
                  ...
                  {isEditing ? (
                    <input
                      {...register('profileData.specialization')}
                      className="input-netflix w-full"
                      placeholder="Enter your specialization"
                    />
                  ) : (
                    <div className="p-3 bg-netflix-gray-dark/30 rounded-lg text-white">
                      {profile.counselorProfile.specialization || 'Not provided'}
                    </div>
                  )}
                </div>
                ...
                <div className="mt-6">
                  ...
                  {isEditing ? (
                    ...
                    <div className="p-3 bg-netflix-gray-dark/30 rounded-lg text-white">
                      {profile.counselorProfile.qualifications || 'Not provided'}
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex justify-end space-x-4 pt-6 border-t border-netflix-gray-dark">
                ...
                <motion.button
                  ...
                  onClick={(e) => {
                    ...
                    // The form will handle submission via onSubmit
                  }}
                >
